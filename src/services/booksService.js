...
exports.addLike = async (userId, bookId) => {
    ...
    try {
        ...
        const updateQuery = `
            UPDATE books
            SET likes = (SELECT COUNT(*) FROM likes WHERE book_id = ?)
            WHERE id = ?
        `;
        await connection.execute(updateQuery, [bookId, bookId]);

        const getLikesQuery = `SELECT likes FROM books WHERE id = ?`;
        const [[{ likes }]] = await connection.execute(getLikesQuery, [bookId]);

        await connection.commit();
        return likes; // 변경된 좋아요 수 반환
    } catch (error) {
